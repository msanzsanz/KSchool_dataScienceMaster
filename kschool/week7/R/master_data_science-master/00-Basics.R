...
setwd("~/Documents/MSS/Personales/Kschool/Kschool/week7/master_data_science-master")
...
# dplyr is one the of most used library to manipulate date
...
# We need to self-assign the output to the original variable if we want to apply the change
...
# Seq.Date() can be used to create a sequence of dates, pretty useful if we want to 
# join to datasets and identify the missing dates on one of them
seq.Date()
...
# substitute $ with !
# El $ indica el final de la cadena de texto, por eso hay que poner las \\, sino lo que hace
# es reemplazar el última caracter
...
# Los vectores tienen que tener todos el mismo tipo
vector <- c(1, "a")
typeof(vector)
