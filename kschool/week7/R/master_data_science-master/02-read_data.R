...
flights <- rbindlist(flights) #raw bind list
...
system.time(system('wc data/flights/2008.csv'))

system.time(nrow(data.table::fread("data/flights/2008.csv", select = 1L, nThread = 2))) # Using fread on the first column
...
system('csvcut ./data/airlines.csv -c Code,Description')
...
#f <- function(x) subset(x, Dest == 'JFK')
...
#################
## Curiosear BigQuery y los APIS / Servicios de Google cloud
##
#################

# se pueden mandar ficheros a través de una dirección de correo, 
# y tener un trigger para crear una VM temporar que procese el email, haga algoy se destruya.

 # Tambiñen se puede crear un cluster de máquinas 