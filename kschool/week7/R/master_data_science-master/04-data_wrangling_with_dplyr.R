...
View(filter(flights, Distance == 11))
...
names(flights)
foo <- filter(flights, Dest == "JFK", TaxiIn < 15, Distance > 1000)
count(foo)


# 2) Combine the Year, Month and DayofMonth variables to create a Date column
foo <- mutate(flights, Date = as.Date(paste (Year, Month, DayofMonth, sep ='-')))

# 3) Result:
select (foo, day(Date) %in% c(0,1))
names(foo)
...
summarise(flights, 
          earliest = min(ArrDelay, na.rm=TRUE), 
          average = mean(ArrDelay, na.rm=TRUE), 
          latest = max(ArrDelay, na.rm=TRUE), 
...
summarise(taxi,
          maximum = max(abs(TaxiIn - TaxiOut)))
...
names(aa)
aa_canceled <- select(aa, CancellationCode )

summarise(aa,
          n_carrier = sum(aa$CancellationCode == "A", na.rm = T),
          n_weather = sum(aa$CancellationCode == "B", na.rm = T),
          n_NAS = sum(aa$CancellationCode == "C", na.rm = T),
          n_security = sum(aa$CancellationCode == "D", na.rm = T))
...
flights %>% 
  ...
  mutate(overnight = ((ArrTime - DepTime) < 0))
...
foo <- flights %>% 
  ...
  mutate_at(vars(ends_with("Delay")), funs(./2))  # el punto indica el valore de la variable
foo
...
boxplot(no_outliers$ActualElapsedTime)
...
filter(foo, CancellationCode == "Carrier")
unique(foo$CancellationCode)
...
flights %>% 
  ...
  arrange(-n) %>% 
  spread(Origin, n) %>% 
  gather("Origin", "n", 2:ncol(.)) %>%  #desde la columna 2 al final
  arrange(-n) 
